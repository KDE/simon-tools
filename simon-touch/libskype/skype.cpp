...
*/
#include "skype.h"
#include "skypedbus/skypeconnection.h"
...
void Skype::connectionDone(int error, int protocolVer) {
	...
	if (error == seSuccess) {//It worked
		...
		closed(crLost);//OK, this is wrong, justclose the connection/attempt and delete the queue
	}
}
...
void Skype::skypeMessage(const QString &message) {
    ...
    emit voiceMailActive(value.toInt());
		...
		if (d->searchFor == "MISSEDMESSAGES") {//Theese are messages we did not read yet
            ...
            if (subType == "TARGET_STATUSES") {
                if (message.contains("TARGET_ACCEPTABLE")) {
                    d->connection << QString("SET SMS %1 BODY %2").arg(smsId).arg(smsBody);
                    d->connection << QString("ALTER SMS %1 SEND").arg(smsId);
                } else
                    qDebug() << "Ignoring SMS target status message: " + message;
            } else if (subType == "STATUS") {
                qDebug() << "SMS Status: " << message;
                if (message.section(' ', 3, 3) == "FAILED") {
                    qWarning() << "Failed to send sms: " << message;
                    d->connection << QString("DELETE SMS %").arg(smsId);
                }
            }
...
void Skype::makeCall(const QString &userId) {
	kDebug(SKYPE_DEBUG_GLOBAL);
        ...
        qDebug() << "Making call: " << QString("CALL %1").arg(userId);
...
void Skype::sendSMS(const QString &userId, const QString& message) {
    kDebug(SKYPE_DEBUG_GLOBAL);
    smsBody = message;
    d->connection << QString("CREATE SMS OUTGOING %1").arg(userId);
}
...
void Skype::fixGroups(bool loadOnly) {
	...
	//Check if groups does not have same names
	if ( ! loadOnly ) {
